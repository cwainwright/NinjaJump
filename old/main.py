...
class Player(pygame.sprite.Sprite):
    ...
    # returns the playes score
    def get_score(self):
        return self.score

    # this increases the players score when a gem is collected
    ...
    def main_loop(self, player_count=1):
        ...
        while alive:
            ...
            for player in players:
                if player.score >= 20 and level2 == False:
                    self.level_index = 1
                    platforms, _ , _, spikes, _ = self.level_setup()
                    players[0].rect.bottomleft = (SCREEN_WIDTH/2, SCREEN_HEIGHT-50)
                    if player_count == 2:
                        players[1].rect.bottomleft = (SCREEN_WIDTH/2 + 100, SCREEN_HEIGHT-50)

                    level2 = True 
                elif player.score >= 40 and level3 == False:
                    self.level_index = 2
                    platforms, _ , _, spikes, _ = self.level_setup()
                    players[0].rect.bottomleft = (SCREEN_WIDTH/2, SCREEN_HEIGHT-50)
                    if player_count == 2:
            ...
            score_label = Label("Player One: " + str(players[0].score), (60, 10), (255, 255, 255))
            score_label.show_label(self.DISPLAYSURF)

            # Score display with two players
            if player_count == 2:
                score_label_2 = Label("Player Two: " + str(players[1].score), (1150, 10), (255, 255, 255))
